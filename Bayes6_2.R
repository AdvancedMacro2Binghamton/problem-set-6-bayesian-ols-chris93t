...
#theta<-c(4,0.05,0.03,0.01 ,.6)


#y_hat<-theta[1] + theta[2]*data$educ+theta[3]*data$exper+theta[4]*data$smsa

#liklihood_result_1<-dnorm(data$lwage, mean = y_hat, sd = theta[5], log = T )

#liklihood_result_2<-lapply(1:length(data$lwage), function (x) dnorm(data$lwage[x], mean = y_hat[x], sd = theta[5], log = T))


#sum(liklihood_result_1==liklihood_result_2)/length(liklihood_result_1)
...
#png('C:/Economics PhD/Third Year/Advanced Macro/HW6/beta wage/pic1.png')
